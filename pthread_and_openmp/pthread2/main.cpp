...
//在这个文件中，我们尝试对单个求交运算进行分割，使每个线程处理的长度为原来的1/n,应该是一种动态的方式
//定义参数存储结构体
struct thread_arg{
...
int main()
{
   ...
   for(int i=0;i<1000;i++){
    ...
    int N=1;
        thread_arg args[8];
        for(int j=0;j<N;j++){
            args[j] = {196916/N*j,196916/N*(j+1)};
        }
    ...
    //pthread_create(&threads[2],NULL,thread_func_2,&args[2]);
    //pthread_create(&threads[3],NULL,thread_func_2,&args[3]);
    //pthread_create(&threads[4],NULL,thread_func_2,&args[0]);
    //pthread_create(&threads[5],NULL,thread_func_2,&args[1]);
    //pthread_create(&threads[6],NULL,thread_func_2,&args[2]);
    //pthread_create(&threads[7],NULL,thread_func_2,&args[3]);
