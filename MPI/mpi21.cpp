...
int main(int argc, char* argv[]) {
    ...
    if (myid != 0) {
        for (int i = 0; i < 1000; ++i) {
            ans[i].resize(100);//固定传输的条数
            MPI_Send(ans[i].data(), ans[i].size(), MPI_INT, 0, i, MPI_COMM_WORLD);
        }
    }
    else {
        int world_size= numprocs;
        // 接收进程
        std::vector<std::vector<std::vector<int>>> ans1(world_size);
        for (int i = 1; i < world_size; ++i) {
            ans1[i].resize(1000);
            for (int j = 0; j < 1000; ++j) {
                ans1[i][j].resize(100);
                MPI_Recv(ans1[i][j].data(), 100, MPI_INT, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    }
