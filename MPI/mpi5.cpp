...
list* lists;
bitset<128> bit_list_now[80000];
bitset<128> bit_list_ans[80000];

bitset<80000> bit_idxs[2000];
...
int main(int argc, char* argv[]) {
   ...
   //由于整块化成位图会导致读写冲突，只能分块进行了
    ...
    for (int i = 0; i < 2000; i++) {
        bit_lists[i] = new bitset<128>[80000];
    }
    ...
    for (int i = myid * 250 / numprocs; i < (myid + 1) * 250 / numprocs; i++) {//标识第几个query
        bitset<80000> idx_now = bit_idxs[querys[i][0]];
