...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        //开始查询
        for (int j = myid; j < min_len; j += numprocs) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ans[i].resize(100); // 假设每个vector<int>大小为100
    }

    // 定义接收缓冲区
    std::vector<int> recv_data;
    if (myid == 0) {
        recv_data.resize(numprocs * 1000 * 100); // 总大小为进程数 * 1000 * 100
    }

    // 将 ans 转换为一维数组以便传输
    std::vector<int> send_data(1000 * 100);
    for (int i = 0; i < 1000; ++i) {
        std::copy(ans[i].begin(), ans[i].end(), send_data.begin() + i * 100);
    }

    // 使用 MPI_Gather 进行数据收集
    MPI_Gather(send_data.data(), 1000 * 100, MPI_INT, recv_data.data(), 1000 * 100, MPI_INT, 0, MPI_COMM_WORLD);
